...
AOD_path <- "user defined path for picking up AOD tif files downloaded from NASA"

AOD <- list.files(path=paste0(AOD_path,"\\2002\\AOD Punjab-20200510T192217Z-001\\AOD Punjab"), 
...
#AOD_Dist <- exact_extract(AOD_06[[2]],DistPak,'sum')
#DistPak[[paste("AOD_total_06")]] <- AOD_Dist
...
for(i in seq(1, nlayers(AOD), 13)) {
  ...
  aod_sum <- exact_extract(AOD[[i]], DistPunjab,'sum')   #sums the values of each tif file's first band, for each district
...
# Write stata file #
write.dta(DistPunjab_sp_df, paste0("...\\Data\\Raw\\aod_fire\\smog_2002_1.dta"))
