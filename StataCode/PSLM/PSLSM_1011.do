*** This do file cleans PSLSMS 2010-11 datasets and restricts them to relevant	***
***	variables.																	*** 

clear
set more off

**	Cleaning and Merging PSLSMS 2010-11	**

* Month and year of interview
use "${...}\PSLM 2010-11\sec_a.dta", clear

tostring 	int_date, replace

gen 		month = substr(int_date, 4,2)
destring 	month, replace
label define monthname 1 "Jan" 2 "Feb" 3 "Mar" 4 "Apr" 5 "May" 6 "Jun" 7 "Jul" ///
			8 "Aug" 9 "Sep" 10 "Oct" 11"Nov" 12"Dec"
label values month monthname	

gen 		yr = substr(int_date,7,2)
destring	yr, replace

gen 		year = 2010 		if yr==10
replace		year = 2011			if yr==11

keep hhcode month year

save "${...}\Temp\2010-11\time.dta", replace 


* Adding hh weights
use "${...}\PSLM 2010-11\weights.dta", clear

rename weight weights

keep hhcode weights

save "${...}\Temp\2010-11\weights.dta", replace 


* Roster of all households
use "${...}\PSLM 2010-11\sec_b_dist.dta", clear

gen male = 0
replace male = 1		if sbq03==1

keep 		hhcode idc province dist age male

rename dist district

save "${...}\Temp\2010-11\roster.dta", replace 


* Education Information
use "${...}\PSLM 2010-11\sec_c_dist.dta", clear

keep 		hhcode idc scq01 scq02 scq03 scq04 scq05 scq06 scq07 
...
replace years_educ = 12 	if scq04==12 | scq04==17
replace years_educ = 16 	if scq04==14 | scq04==18 | scq04==19 | scq04==20 | scq04==21 // Assuming BA/BSc/Degree in engineering, computers  //
																			 // and agriculture equals 16 years of education	 //
replace years_educ = 17 	if scq04==16									// Assuming MA/MSc, MBBS equals 17 years of education //
replace years_educ = 21 	if scq04==22									// Assuming PhD/M.Phil equals 21 years of education //
replace years_educ = .a 	if scq04==23									// Taking "others" as missing vallues, total no of "others" //
																			// observations is 80 //
label var years_educ 		"Total completed years of education"

keep	hhcode idc years_educ ever_school read_write
			
save "${...}\Temp\2010-11\education.dta", replace 


* Employment Information
use "${...}\PSLM 2010-11\sec_e_dist.dta", clear
...
save "${...}\Temp\2010-11\employment.dta", replace 


* Health Information
use "${...}\PSLM 2010-11\section_d.dta", clear	
...
save "${...}\Temp\2010-11\health.dta", replace 


* Merging all datasets
use "${...}\Temp\2010-11\roster.dta", clear

merge 1:1 hhcode idc using "${...}\Temp\2010-11\education.dta"
drop if _merge==2
drop _merge	

merge 1:1 hhcode idc using "${...}\Temp\2010-11\employment.dta"
drop if _merge==2
drop _merge

merge 1:1 hhcode idc using "${...}\Temp\2010-11\health.dta"
drop if _merge==2
drop _merge

merge m:1 hhcode using "${...}\Temp\2010-11\time.dta"
drop if _merge==2
drop _merge

merge m:1 hhcode using "${...}\Temp\2010-11\weights.dta"
drop if _merge==2
drop _merge
...
gen gdp_deflator_10 = 162.435513755632 / 100
gen gdp_deflator_11 = 194.345403739772 / 100

gen cpi_10 = 100 / 100
gen cpi_11 = 111.916769465206 / 100

gen inflation_10 = 0.1388113926
gen inflation_11 = 0.1191676947

gen 	real_income = income_m / (1 + inflation_10)		if year==2010
replace real_income = income_m / (1 + inflation_11)		if year==2011
...
recode  district	(111=160) (122=150) (126=170) (127=180) (128=190) (129=200) ///
					(130=210) (131=220) (132=230) (133=240)
recode 	district	(112=111) (113=112) (114=113) (115=114) (116=115) (117=116) ///
					(118=117) (119=118) (120=119) (121=121) (123=122) (124=124) ///
					(125=125) (126=127) (127=128) (129=130) (130=131) (131=132) ///
					(132=133) (133=134) 
recode	district	(134=123) (135=126) (136=120)
recode	district	(150=135) (160=136) (170=127) (180=128) (190=129) (200=130) ///
					(210=131) (220=132) (230=133) (240=134)

label define dist2 100 "Islamabad" 101 "Attock" 102 "Rawalpindi" 103 "Jehlum" 104	"Chakwal" ///
...
label values district dist2

save "${...}\PSLSM_1011_final.dta", replace
